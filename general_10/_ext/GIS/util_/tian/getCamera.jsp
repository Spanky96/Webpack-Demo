...
if (StringUtils.isNotBlank(currentTypeId) && !"null".equalsIgnoreCase(currentTypeId)) {
	...
	List<GenericValue> ents = NS.findEnts("ExtXKCameras", filter);
	for (GenericValue ent: ents) {
		JSONObject obj = new JSONObject();
		obj.put("id", ent.getString("cameraId"));
		obj.put("name", ent.getString("title"));
		obj.put("addr", Global.getFieldLabel(ent, "addr"));
		String lng = ent.getString("bd09").split(",")[0];
		String lat = ent.getString("bd09").split(",")[1];
		Gps gps = PositionUtil.bd09_To_Gps84(Double.parseDouble(lat), Double.parseDouble(lng));
		obj.put("lng", gps.getWgLon());
		obj.put("lat", gps.getWgLat());
		array.put(obj);
	}
}
