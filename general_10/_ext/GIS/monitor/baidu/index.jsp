...
List<GenericValue> list = NS.findAll("ExtXKCameraType", UtilMisc.toList("orderNo"));
%>
<!DOCTYPE html>
<html>
<head>
...
function getElement(currentGridId, currentTypeId) {
	$.getJSON("/general_10/_ext/GIS/util/baidu/getCamera.jsp?currentGridId="+currentGridId+"&currentTypeId="+currentTypeId, function(json) {
		if (json.length > 0) {
			for (var i=0; i<json.length; i++) {
				...
				(function() {
					...
					marker.addEventListener("click", function(){
						showElement(obj.id, obj.addr+obj.name);
					});
				})();
			}
		}
...
function showElement(id, name) {
	layer.open({
		type: 2,
		title: name,
		content: '/general_10/_ext/GIS/util/showCamera.jsp?id='+id,
		area: ['640px', '480px'],
	});
}
</script>
...
var zNodes = [
	{id: '00', pId: null, name: "监控类型", open: true},
	<%
	for (GenericValue ent : list) {
	%>
	{id: '<%=ent.getString("typeId")%>', pId: '00', name: "<%=Global.getFieldLabel(ent, "typeName")%>", open: true, icon: '/scripts/ztree/zTreeStyle/img/diy/11.png'},
	<%
	}
	%>
];
