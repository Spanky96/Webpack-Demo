...
function b_mOver_ns4(nr) {
  ...
  l.write("' border=0><font color=white>");
  l.write(this.lbl[nr]+"</FONT></A></SPAN>");
  l.close();
  this.sta[nr]=1;
}
...
function createPanel(name,caption) {
  ...
  this.ftop=26;                    // actual panel scroll position
...
function b_writeStyle() {
  ...
  document.write('.button {line-height:20px;text-align:center;font-weight:bold;');
  document.write(' font-size:12px; cursor:hand; background:url(/images/menu/M1.gif);}');

  document.write('.noLine {text-decoration:none;}');
  document.write('.welcome{line-height:25px;text-align:center;');
  document.write(' font-size:12px; background:url(/general/users/images/mystatus.gif);}');

  document.write('.imgB {color:white; font-size:12px;}');
  ...
  if (bt.ns4) {                 //draw OutlookBar for ns4
    ...
    for (i=0;i<this.panels.length;i++) {
      document.write('<Layer name='+this.name+'_panel'+i+' width='+this.width);
       document.write(' top='+i*panelHeight+' bgcolor='+this.bgcolor);
       document.write(' clip="0,0,'+this.width+',');
       document.write(this.height-(this.panels.length-1)*panelHeight+'">');
       ...
       document.write('<Layer top=38 width='+this.width+'>');
        ...
        for (j=0;j<this.panels[i].img.length;j++) {
          ...
          document.write(this.panels[i].lbl[j]+"</FONT></A></SPAN>");
       ...
       document.write(this.width+',panelHeight" bgcolor=silver class=button ');
       ...
       document.write('width=16 height=16 src=/images/menu/M2.gif border=0>');
       document.write('</A></LAYER><Layer top=');
       document.write((this.height-(this.panels.length)*panelHeight)+'<Layer top=');
       document.write((this.height-(this.panels.length)*panelHeight)+' left=');
       ...
       document.write('src=/images/menu/M3.gif border=0></A></LAYER>');
    ...
    h=this.height-((this.panels.length-1)*panelHeight)
    ...
    for (i=0;i<this.panels.length;i++) {
      ...
      for (j=0;j<this.panels[i].img.length;j++) {
        if (bt.op5) {
          ...
          document.write('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <img width=16 height=16 align=absmiddle src='+this.panels[i].img[j]+'>');
          document.write(' <a href=# onclick='+this.panels[i].act[j]+'><font size=2>'+this.panels[i].lbl[j]+'</font></a></DIV>');
        }
        mtop=mtop+this.buttonspace;
      }
      ...
      //two layers for scroll-up -down buttons
      document.write('<DIV id='+this.name+'_panel'+i);
      document.write('_m1 style="position:absolute; top:30; left:');
      document.write((this.width-20)+';"><A href="#" onClick="');
      document.write(this.panels[i].v+'.down(16);this.blur();return false;" ');
      document.write('onmouseOver="'+this.panels[i].v+'.clear();">');
      document.write('<img width=16 height=16 src=/images/menu/M2.gif border=0>');
      ...
      document.write('onmouseOver="'+this.panels[i].v+'.clear();">');
      document.write('<img width=16 height=16 src=/images/menu/M3.gif border=0>');
      document.write('</A></DIV>');
    ...
    if (bt.op5) {
      ...
      for (i=0;i<this.panels.length;i++) {
        ...
        document.write(');" style="position:absolute; left:0; top:0; width:');
        document.write((this.width-c)+'; height:'+(panelHeight-c)+';">');
...
var l
  ...
  for (i=0;i<l;i++) {
    if (i>nr) {
      this.panels[i].obj.top=this.height-((l-i)*panelHeight)-1;
    }
    else {
      this.panels[i].obj.top=i*panelHeight;
    }
  }
}
...
var o
  ...
  for (i=0;i<l;i++) {
    if (i>nr) {
      this.panels[i].obj.style.top=this.height-((l-i)*panelHeight);
      ...
      if (bt.op5) {
        ...
        this.panels[i].objc.style.top=this.height-((l-i)*panelHeight);
      }
    }
    else {
    if(i==0) this.panels[i].obj.style.top=0
    else this.panels[i].obj.style.top=i*panelHeight+5;
      ...
      if (bt.op5) {
        ...
        this.panels[i].objc.style.top=i*panelHeight;
      }
    }
  }
}
   ...
   if (bt.ns4)
     ...
     if (bt.op5 && (width!=this.width))
       ...
       for (i=0; i<this.panels.length; i++) {
         ...
         this.panels[i].objm2.style.top=(this.height-(this.panels.length)*panelHeight);
...
function createOutlookBar(name,x,y,width,height) {
  ...
  this.bgcolor="#FFFFFF";                   // bar background color
  this.pagecolor="white";               // page bgcolor (op5!)
  this.buttonspace=20;                     // distance of panel buttons
