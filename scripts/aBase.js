...
function check(it){
  ...
  }else if(it.getAttribute("CHECKMONEY")){
	return checkMoney(it,it.getAttribute("CHECKMONEY"));
  }
  return true;
}

function checkMoney(it,str){
  if (!re_money.exec(trim(it.value))){
...
var re_mobile=/^((86)|(0))?((1(3|4|5|8)\d{9}))$/;
...
var re_money = /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/;
...
function selectDate(tag) {
	...
	if(info=="edge"||info=="Chrome"){
		var w = centWinNo("/scripts/calendar0.jsp?dateStr=" + dateStr + "&timeStr=" + timeStr,280,240);
...
function selectTime(tag) {
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/scripts/time.jsp?dateStr=" + dateStr + "&timeStr=" + timeStr;
		var w = centWinNo(url,150,120);
	    w.returnValue = function(str){
	        theInput.value = str;
	    };
	}else{
...
function selectImg(tag, path, width, height) {
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/common_0/selectImg/index.jsp?path=" + path;
		var w = centWinNo(url,w,h);
	    w.returnValue = function(str){
	    	theInput.value = str;
	    };
	}else{
...
function selectLargeImg(tag, path, width, height) {
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/common_0/selectImg/indexLarge.jsp?path=" + path;
...
function selectEnt(tag, entityName, topId) {
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/common_0/selectEnt/index.jsp?entityName=" + entityName;
		var w = centWinNo(url,300,440);
	    w.returnValue = function(str){
	    	returnValue = str;
...
function selectEnts(tag, entityName ,topId) {
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/common_0/selectEnts/index.jsp?entityName=" + entityName + "&relIds=" + relIds + topStr;
		var w = centWinNo(url,300,440);
	    w.returnValue = function(str){
	    	returnValue = str;
	        if (returnValue && returnValue != null) {
...
function selectUser(tag) {
	...
	var href="/common_0/selectUser/index.jsp";
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var w = centWinNo(href,350,310);
	    w.returnValue = function(str){
	    	returnValue = str;
	        if (returnValue && returnValue != null) {
	    		values = returnValue.split(",");
		...
		returnValue = window.showModalDialog(href, relIds, "scroll:0;status:1;help:0;resizable:0;dialogWidth:350px;dialogHeight:310px");
	}
...
function selectUsers(tag) {
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/common_0/selectUsers/index.jsp?relIds=" + relIds.value;
		var w = centWinNo(url);
	    w.getValue = function(str){
	    	returnValue = str;
	        if (returnValue && returnValue != null) {
				values = returnValue.split("|");
				if (input1.type && input1.type == "hidden") {
		...
		returnValue = window.showModalDialog("/common_0/selectUsers/index.jsp?relIds=" + relIds.value, relIds, "scroll:0;status:1;help:0;resizable:0;dialogWidth:580px;dialogHeight:320px");
		if (returnValue && returnValue != null) {
			values = returnValue.split("|");
			if (input1.type && input1.type == "hidden") {
				input1.value = values[0];
...
function selectSpaceUsers(tag,online) {
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/common_0/selectSpaceUsers/index.jsp?online="+online;
		var w = centWinNo(url);
	    w.returnValue = function(str){
	    	returnValue = str;
	        if (returnValue && returnValue != null) {
...
function selectFile(queryStr,item){
  ...
  var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		itemOption = item;
		centWinNo('/general_0/netfile/attach/index.jsp?'+queryStr);
	}else{
		 if(window.showModelessDialog)
			  window.showModelessDialog('/general_0/netfile/attach/index.jsp?'+queryStr, item, 'scroll:0;status:1;help:0;resizable:0;dialogWidth:800px;dialogHeight:400px');
		  else 
			  window.showModalDialog('/general_0/netfile/attach/index.jsp?'+queryStr, item, 'scroll:0;status:1;help:0;resizable:0;dialogWidth:800px;dialogHeight:400px');
	}
}

function editDoc(name,docPath){
	centWin("/general_0/workflow/office/editContent.jsp?filePath="+encodeURI(docPath)+"&fromEdit=1");
}

function viewDoc(docPath){
	centWin("/general_0/workflow/office/viewContent.jsp?filePath="+encodeURI(docPath));
}
...
function publicMailSelectFile(queryStr,item){
	  ...
	  var info = getExplorer();
		if(info=="edge"||info=="Chrome"){
			itemOption = item;
			centWinNo('/general/netfile/attach/emailindex.jsp?'+queryStr,400,280);
		}else{
...
function delAttach(itemName,url,infoId){
	  if(confirm("是否确认删除?")){
		  
		  if(checkValueNull(infoId)){
			  delCaseFile(itemName,url,infoId)
		  }
...
function checkValueNull(info){
	if(info==null||info==""||info=='null'||typeof(info)=='undefined'){
		return false;
	}else{
		return true;
	}
...
function delCaseFile(itemName,url,infoId){
	var templateId = itemName.substring(itemName.indexOf("_")+1);
	...
	$.ajax({ 
		type:"post", 
		url: "/mobile/zhzf/ajaxApi/delAttach/delCaseAttachAjax.jsp", 
		 dataType:"json",
		 data:param,
...
function selectObjs(tag,namespace) {
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/common_0/selectObjs/index.jsp";
		var w = centWinNo(url,580,320);
	    w.returnValue = function(str){
	    	returnValue = str;
	    	if (returnValue && returnValue != null) {
	    		values = returnValue.split("|");
...
function selectEntTitles(tag,entityName){
  var input2 = getPreviousInput(tag);
	...
	var info = getExplorer();
	if(info=="edge"||info=="Chrome"){
		var url = "/common_0/selectEntTitles/index.jsp?entityName=" + entityName + "&relIds=" + relIds;
		var w = centWinNo(url,280,440);
	    w.returnValue = function(str){
	    	returnValue = str;
	    	if (returnValue && returnValue != null) {
	    		values = returnValue.split("|");
...
//编辑Office内容
function editOffice(filePath){
	centWin("/general_10/office/editOffice.jsp?filePath="+encodeURI(filePath));
}

//查看Office内容
function viewOffice(filePath){
	centWin("/general_10/office/viewOffice.jsp?filePath="+encodeURI(filePath));
}

//查看PDF内容
function viewPDF(filePath){
	centWin("/general_10/office/viewPDF.jsp?filePath="+(filePath));
}

//查看img内容
function viewImg(filePath){
	centWin("/general_10/office/viewImg.jsp?filePath="+(filePath));
}
